...
app.post("/api/books/AddBook", multer().none(), async (req, res) => {
  try {
    // 1. Generate a simple ID (In production, usually use MongoDB's _id)
    const numOfDocs = await database.collection("Books").countDocuments();

    // 2. Insert with the new fields
    // NOTE: We use Number() to ensure numeric data is saved correctly for sorting/math
    await database.collection("Books").insertOne({
      id: String(numOfDocs + 1),
      title: req.body.title,
      description: req.body.description, // Matches the frontend formData.append("description")
      author: req.body.author,           // New field
      yearPublished: Number(req.body.yearPublished) || 0, // New field (saved as Number)
      price: Number(req.body.price) || 0,
    });

    // 3. Senior Tip: Return a consistent response
